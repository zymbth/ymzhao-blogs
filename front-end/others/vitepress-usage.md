...
可以简单理解为网站布局，不改布局的话直接跳过吧，或者有了全面的了解后再来研究如何[自定义主题](https://vitepress.dev/zh/guide/custom-theme)
...
`Layout.vue`: 实际上，这里并没有重写 `Layout`，仅仅只是通过其插槽渲染评论组件而已。当然，你完全可以重写以完全自定义。更多插槽参见官网[扩展默认主题-布局插槽](https://vitepress.dev/zh/guide/extending-default-theme#layout-slots)
